...
class HotelManagementSystem:
    ...
    def add_user(self):
        self.clear_content_frame()
        tk.Label(self.content_frame, text="Add User", font=("Arial", 18)).pack(pady=10)

        labels = ['Username', 'Password', 'Name', 'Surname', 'Position', 'Role']
        entries = {}
        for i, label in enumerate(labels):
            tk.Label(self.content_frame, text=label, font=("Arial", 14)).pack(pady=5)
            entries[label] = tk.Entry(self.content_frame, font=("Arial", 14))
            entries[label].pack(pady=5)

        def submit():
            cursor = self.connection.cursor()
            query = "INSERT INTO Users (username, password, name, surname, position, role) VALUES (%s, %s, %s, %s, %s, %s)"
        ...
        tk.Button(self.content_frame, text="Submit", command=submit, font=("Arial", 14)).pack(pady=20)

    def delete_user(self):
        self.clear_content_frame()
        tk.Label(self.content_frame, text="Delete User", font=("Arial", 18)).pack(pady=10)

        tk.Label(self.content_frame, text="Username", font=("Arial", 14)).pack(pady=5)
        username_entry = tk.Entry(self.content_frame, font=("Arial", 14))
    ...
    def update_user(self):
        self.clear_content_frame()
        tk.Label(self.content_frame, text="Update User", font=("Arial", 18)).pack(pady=10)

        tk.Label(self.content_frame, text="Username", font=("Arial", 14)).pack(pady=5)
        username_entry = tk.Entry(self.content_frame, font=("Arial", 14))
        username_entry.pack(pady=5)

        labels = ['Password', 'Name', 'Surname', 'Position', 'Role']
        entries = {}
        for label in labels:
            tk.Label(self.content_frame, text=label, font=("Arial", 14)).pack(pady=5)
            entries[label] = tk.Entry(self.content_frame, font=("Arial", 14))
        ...
        def submit():
            cursor = self.connection.cursor()
            query = "UPDATE Users SET password=%s, name=%s, surname=%s, position=%s, role=%s WHERE username=%s"
            values = [entries[label].get() for label in labels] + [username_entry.get()]
            cursor.execute(query, values)
            self.connection.commit()
            cursor.close()
    ...
    def view_users(self):
        self.clear_content_frame()
        tk.Label(self.content_frame, text="View Users", font=("Arial", 18)).pack(pady=10)

        columns = ('user_id', 'username', 'name', 'surname', 'position', 'role')
        tree = ttk.Treeview(self.content_frame, columns=columns, show='headings')
        tree.pack(fill=tk.BOTH, expand=True)
        ...
        for user in users:
            tree.insert('', tk.END, values=(user['user_id'], user['username'], user['name'], user['surname'], user['position'], user['role']))
    ...
    def admin_rooms_options(self):
        return {
            ...
            "Update Room": self.update_room,
            "View Rooms": self.view_rooms,
        }

    def add_room(self):
        self.clear_content_frame()
        tk.Label(self.content_frame, text="Add Room", font=("Arial", 18)).pack(pady=10)

        labels = ['Room Number', 'Room Type', 'Price', 'Status']
        entries = {}
        for i, label in enumerate(labels):
            tk.Label(self.content_frame, text=label, font=("Arial", 14)).pack(pady=5)
            entries[label] = tk.Entry(self.content_frame, font=("Arial", 14))
        ...
        def submit():
            cursor = self.connection.cursor()
            query = "INSERT INTO Rooms (room_number, room_type, price, status) VALUES (%s, %s, %s, %s)"
            values = [entries[label].get() for label in labels]
            cursor.execute(query, values)
            self.connection.commit()
            cursor.close()
    ...
    def delete_room(self):
        self.clear_content_frame()
        tk.Label(self.content_frame, text="Delete Room", font=("Arial", 18)).pack(pady=10)

        tk.Label(self.content_frame, text="Room Number", font=("Arial", 14)).pack(pady=5)
        room_number_entry = tk.Entry(self.content_frame, font=("Arial", 14))
        room_number_entry.pack(pady=5)

        def submit():
            cursor = self.connection.cursor()
            query = "DELETE FROM Rooms WHERE room_number=%s"
            cursor.execute(query, (room_number_entry.get(),))
            self.connection.commit()
            cursor.close()
    ...
    def update_room(self):
        self.clear_content_frame()
        tk.Label(self.content_frame, text="Update Room", font=("Arial", 18)).pack(pady=10)

        tk.Label(self.content_frame, text="Room Number", font=("Arial", 14)).pack(pady=5)
        room_number_entry = tk.Entry(self.content_frame, font=("Arial", 14))
        room_number_entry.pack(pady=5)

        labels = ['Room Type', 'Price', 'Status']
        entries = {}
        for label in labels:
            tk.Label(self.content_frame, text=label, font=("Arial", 14)).pack(pady=5)
            entries[label] = tk.Entry(self.content_frame, font=("Arial", 14))
        ...
        def submit():
            cursor = self.connection.cursor()
            query = "UPDATE Rooms SET room_type=%s, price=%s, status=%s WHERE room_number=%s"
            values = [entries[label].get() for label in labels] + [room_number_entry.get()]
            cursor.execute(query, values)
            self.connection.commit()
            cursor.close()
